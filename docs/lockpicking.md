...
| Lockpick | `ACE_key_lockpick` | kit | ACE's vehicle lockpick, if *ACE lockpick counts as a kit* is on — from the built-in Door menu only, since tsp_breach's own actions accept only its items |
...
paperclips — but they still work, and saved loadouts that contain them keep
working.
...
2. **Rake.** Press <kbd>R</kbd> (or button 1) to rake. A stroke takes about a
   ...
   - **Needle above the band** (too much tension): the pins bind and can slip —
     half the time at Normal, a mistake.
...
3. **Don't force it.** Holding the turn where the plug has stopped builds
   **strain**. Let go and strain eases. Too much strain and the plug springs back —
   a mistake.
...
Doors that already have a lock value — set by the mission, a script or a player —
are never overwritten, and glass doors are never locked at random.
