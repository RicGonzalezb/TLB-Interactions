<#
.SYNOPSIS
    Builds @TLB Interactions from source.
...
$ReleaseDir = Join-Path $Root "release\@TLB Interactions"
...
# from each other (defusal takes its version from main), so the whole tree has
# to be in place before the preprocessor runs.
$components = @()
...
if ($Deploy) {
    ...
    $dest = Join-Path $Deploy "@TLB Interactions"
    Write-Host ""
